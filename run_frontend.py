...
def info_whisky(name:str):
    ...
    st.write(f'price:\t[CAD] {price[0]} ~ {price[1]}')
    st.write(f'type :\t{Type[condition].iloc[0]}')
    st.write(f'link :\t[{name}]({links[condition].iloc[0]})')
...
init = {
    ...
    'value': 0,
    'whisky_list': {},
    'df_final': pd.read_csv('/opt/ml/workspace/src/src_processed/total_df.csv', sep='$'),
}
